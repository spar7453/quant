...
impl<'a> European<'a> {
  pub fn new(option_type: &'a OptionType, k: f64, t: f64) -> European<'a> {
    Self { option_type, k, t }
  }

  pub fn payoff(option_type: &'a OptionType, s: f64, k: f64) -> f64 {
    match option_type {
...
impl<'a> pricing::BinomialTree for European<'a> {
  ...
  fn binomial_tree_theo(&self, mv_ref: &MarketVariables, nstep: usize) -> f64 {
    ...
    let mut layer: Vec<f64> = (0..last_layer_size).map(|down| {
        let s_t: f64 = European::node_price(s, u, d, down, nstep);
...
impl<'a> pricing::Analytic for European<'a> {
...
impl<'a> pricing::MonteCarlo for European<'a> {
